<?php
//   /opt/zfaka/code/application/library/Pay/alpaywxpay
...
//            $url = $payconfig['configure3'] . 'submit.php?' . $this->createLinkstring($config);
            $url = $payconfig['configure3'] . '/wp-admin/admin-ajax.php?action=cashier&price=' . strval($params['money']) . '&title=' . $params['productname'] . '&order_no=' . $params['orderid'] . '&notify_url=' . $params['weburl'] . '/product/notify/?paymethod=' . $this->paymethod;
            if($url){
                $result = array('type' => 0, 'subjump' => 1, 'subjumpurl' => $url , 'paymethod' => $this->paymethod, 'qr' =>  'http://zhiboluzhi.cn/generate_Image?qr_url=' . urlencode($url) , 'payname' => $payconfig['payname'], 'overtime' => $payconfig['overtime'], 'money' => $params['money']);
                return array('code' => 1, 'msg' => 'success', 'data' => $result);
            }else{
                return array('code'=>1001,'msg'=>'支付接口请求失败','data'=>'');
            }
        } catch (\Exception $e) {
            return array('code' => 1000, 'msg' => $e->getMessage(). '12geds', 'data' => '');
        }
    }
...
//            $md5Sigm = md5($this->createLinkstring($params) . $payconfig['app_secret']);
            ...
            if (1) {
                //成功
                //商户订单号
                $config = array('paymethod' => $this->paymethod, 'tradeid' => $params['aoid'], 'paymoney' => $params['pay_price'], 'orderid' => $params['order_id']);
                $notify = new \Pay\notify();
